...
import { KineticPauseControl } from "@/components/home/KineticPauseControl";
import {
  KineticTextField,
  type KineticTextPhase,
} from "@/components/home/KineticTextField";
import {
  FRICTION_DIRECTIONS,
  type FrictionDirection,
} from "@/components/home/kineticTextCopy";
...
interface PendingQueryGate {
  query: string;
  normalizedQuery: string;
}

interface PendingReadingGate {
  passageId: string;
  passageText: string;
  target: ActiveReadingTarget;
}

const QUERY_FRICTION_SESSION_PREFIX = "infidao:home:query-friction:";
const READING_FRICTION_SESSION_PREFIX = "infidao:home:reading-friction:";
...
function normalizeFrictionQuery(query: string) {
  return query.trim().replace(/\s+/g, " ").toLocaleLowerCase();
}

function shouldShowShortQueryGate(query: string) {
  const normalizedQuery = normalizeFrictionQuery(query);

  if (!normalizedQuery) {
    return false;
  }

  const cjkCount = (normalizedQuery.match(/\p{Script=Han}/gu) ?? []).length;
  const latinLetterCount = (normalizedQuery.match(/[a-z]/giu) ?? []).length;
  const words = normalizedQuery.split(/\s+/u).filter(Boolean);

  if (cjkCount >= 1 && cjkCount <= 4) {
    return true;
  }

  if (cjkCount === 0 && latinLetterCount >= 1 && latinLetterCount <= 12) {
    return true;
  }

  return cjkCount === 0 && words.length >= 1 && words.length <= 2;
}

function readSessionFlag(prefix: string, key: string) {
...
function writeSessionFlag(prefix: string, key: string) {
  if (typeof window === "undefined") {
    return;
  }
  ...
  try {
    ...
    // Session storage can be unavailable in private contexts; the gate remains usable.
  }
}
...
export function HomeEntryExperience() {
  ...
  const [isKineticPaused, setIsKineticPaused] = useState(false);
  const [pendingQueryGate, setPendingQueryGate] = useState<PendingQueryGate | null>(null);
  const [pendingReadingGate, setPendingReadingGate] = useState<PendingReadingGate | null>(null);
  const [selectedDirection, setSelectedDirection] = useState<FrictionDirection | null>(null);
  const [transitionHint, setTransitionHint] = useState<KineticTextPhase | null>(null);
  ...
  const hasActiveFrictionGate = pendingQueryGate !== null || pendingReadingGate !== null;
  ...
  const completeQueryGate = (direction: FrictionDirection | null) => {
    if (!pendingQueryGate) {
      return;
    }

    writeSessionFlag(QUERY_FRICTION_SESSION_PREFIX, pendingQueryGate.normalizedQuery);
    setSelectedDirection(direction);
    setPendingQueryGate(null);
    void performSearch(pendingQueryGate.query);
  };
  ...
  const handleSearch = (queryOverride?: string) => {
    ...
    const normalizedQuery = normalizeFrictionQuery(nextQuery);
    setTransitionHint(null);

    if (
      shouldShowShortQueryGate(nextQuery) &&
      !readSessionFlag(QUERY_FRICTION_SESSION_PREFIX, normalizedQuery)
    ) {
      cancelAnnotationRequest();
      setSearchQuery(nextQuery);
      setPendingReadingGate(null);
  ...
  const performAnnotate = async (passageId: string, passageText: string) => {
    if (!searchQuery.trim()) {
      return;
    }
  ...
  const completeReadingGate = () => {
    if (!pendingReadingGate) {
      return;
    }

    writeSessionFlag(READING_FRICTION_SESSION_PREFIX, pendingReadingGate.passageId);
    setPendingReadingGate(null);
    void performAnnotate(pendingReadingGate.passageId, pendingReadingGate.passageText);
  };

  const handleAnnotate = (passageId: string, passageText: string) => {
    if (!searchQuery.trim()) {
      return;
    }
    ...
    if (
      activeRootPassageId !== passageId &&
      annotation?.passageId !== passageId &&
      !readSessionFlag(READING_FRICTION_SESSION_PREFIX, passageId)
    ) {
      setPendingQueryGate(null);
      setPendingReadingGate({
        passageId,
        passageText,
        target: buildRootReadingTarget(searchResults, passageId, passageText),
      });
      return;
    }

    void performAnnotate(passageId, passageText);
  };
  ...
  const handleAnnotationRetry = () => {
    ...
    if (retryTarget.kind === "root") {
      void performAnnotate(retryTarget.passageId, retryTarget.passageText);
      return;
    }
  ...
  const homePhase: KineticTextPhase =
    transitionHint === "back" && !hasSearched && !searchQuery.trim()
      ? "back"
      : hasActiveFrictionGate
        ? "friction"
        : isSearching
          ? "submitting"
          : searchError
            ...
            : wikiStack.length > 1
              ...
              : hasAnnotationSurface
                ...
                : hasSearched
                  ? "results"
                  : isSearchInputFocused || searchQuery.trim()
                    ? "typing"
                    : "idle";

  return (
    <div
      data-home-phase={homePhase}
      className="relative min-h-screen min-h-[100dvh] overflow-hidden ritual-shell text-paper"
    >
      <div className="pointer-events-none absolute inset-0 overflow-hidden">
        <div className="absolute inset-0 page-vignette" />
        <KineticTextField
          paused={isKineticPaused}
          reducedMotion={prefersReducedMotion}
          phase={homePhase}
        />
        <div className="absolute left-1/2 top-1/2 w-full -translate-x-1/2 -translate-y-1/2 text-center transition-opacity duration-500">
          <span className="font-classic text-[8rem] leading-none text-stone-900/30 blur-[3px] md:text-[14rem] lg:text-[20rem]">
            {atmosphere.main}
          </span>
        </div>
        {atmosphere.accents.map((accent, index) => (
          <div
            key={`${accent}-${index}`}
            className={`absolute font-classic text-stone-800/20 blur-[1px] transition-opacity duration-500 ${
              index === 0
      ...
      <div className="fixed right-4 top-4 z-30">
        <KineticPauseControl
          paused={isKineticPaused}
          reducedMotion={prefersReducedMotion}
          onToggle={() => setIsKineticPaused(current => !current)}
        />
      </div>
      ...
      {pendingQueryGate && (
        <DesirableFrictionGate
          mode="query"
          query={pendingQueryGate.query}
          directions={FRICTION_DIRECTIONS}
          onChooseDirection={completeQueryGate}
          onContinue={() => completeQueryGate(null)}
          onSkip={() => completeQueryGate(null)}
        />
      )}
      ...
      {pendingReadingGate && (
        <DesirableFrictionGate
          ...
          reducedMotion={prefersReducedMotion}
          onContinue={completeReadingGate}
          onSkip={completeReadingGate}
        />
      )}

      <main className="relative z-10">
        {!hasSearched && (
          <section className="flex min-h-screen min-h-[100dvh] flex-col items-center px-6 pb-8 pt-[12vh] md:justify-center md:py-16">
            <div className="mb-6 inline-flex items-center border-b border-stone-800 px-4 py-1 text-xs tracking-[0.32em] text-stone-500">
              INFIDAO
            </div>
            ...
            <div className="mt-10 w-full max-w-3xl md:mt-14">
              <SearchBar
                ...
                disabled={hasActiveFrictionGate}
        ...
        {hasSearched && (
          <section className="min-h-screen min-h-[100dvh] px-6 pb-16 pt-4 md:px-8 md:pt-8">
            ...
            >
              ...
              {selectedDirection && (
                <p
                  data-testid="selected-direction-frame"
                  className="mx-auto mt-3 max-w-xl text-sm leading-7 text-zen/90"
                >
                  以「{selectedDirection.label}」方向入经，原问不改。
                </p>
              )}
