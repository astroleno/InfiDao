...
const SimonRogersEffect = () => {
  ...
  useEffect(() => {
    const updateRotations = () => {
      const elements = containerRef.current?.querySelectorAll('.text-line');
      if (!elements) return;
      ...
      elements.forEach((element) => {
        ...
        // 圆柱体投影效果 - rotateX
        const maxRotateX = 80;
        const rotateX = distanceFromCenter * maxRotateX;
        
        // 斜向旋转效果 - rotateZ
        const maxRotateZ = 12;
        const rotateZ = distanceFromCenter * maxRotateZ;
        
        // 透视缩放
        const minScale = 0.7;
        const scale = 1 - (Math.abs(distanceFromCenter) * (1 - minScale));
        
        // 透明度
        const minOpacity = 0.2;
        const opacity = 1 - (Math.abs(distanceFromCenter) * (1 - minOpacity));
        
        (element as HTMLElement).style.transform = 
          `perspective(600px) rotateX(${rotateX}deg) rotateZ(${rotateZ}deg) scale(${scale})`;
        (element as HTMLElement).style.opacity = `${Math.max(0, opacity)}`;
      });
    };

    updateRotations();
    const interval = setInterval(updateRotations, 16);
      ...
      >
        ...
        <div className="py-40">
          {content.map((line, index) => (
            ...
            >
              <p 
                className="max-w-4xl text-center font-serif text-lg leading-relaxed md:text-xl lg:text-2xl"
                style={{ color: 'black' }}
              >
                {line || '\u00A0'}
              </p>
