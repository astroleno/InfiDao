...
仓库已有 Jest 单元、集成和 UI 回归测试，以及针对生产 API 的 release smoke；但没有可在真实浏览器中重复执行、并可接入 CI 的正式端到端测试。当前 Reboot MVP 的公开主流程是：
...
- CI 中对已构建的 standalone production server 执行同一套测试。
...
- 设置 `E2E_BASE_URL`：不启动额外服务，直接连接指定地址。CI 将其指向现有 standalone production server `http://127.0.0.1:3001`。

这样本地无需先构建，CI 又能验证真实发布构建；两种环境共享相同 spec，避免形成两套行为契约。
...
- CI 中使用单 worker 和一次重试，降低真实语料冷启动与共享服务造成的波动。
...
- `npm ci` 后安装 Playwright Chromium 及其 Linux 运行依赖。
- 保留现有类型、Lint、Jest、质量、构建和 release smoke 门禁。
- 在 standalone server 通过 release smoke 后，使用相同 server 执行 `E2E_BASE_URL=http://127.0.0.1:3001 npm run test:e2e`。
- 失败时输出 server log，并通过 `actions/upload-artifact@v4` 上传 `test-results/` 与 `playwright-report/`。

### `README.md`

- 在验证命令和 Active Commands 中加入 `npm run test:e2e`。
- 说明首次本地执行需要安装 Chromium：`npx playwright install chromium`。
- 说明 CI 对 production standalone 执行 E2E，本地默认使用开发服务。

## 数据与环境

- 搜索使用仓库提交的本地 corpus、embeddings 和 graph artifact。
- 注语使用项目现有 deterministic fallback。Playwright 启动的本地服务和 CI standalone 服务会显式清空受支持的 annotation API key 环境变量，避免开发者 `.env.local` 或 CI secret 改变测试路径。
...
- CI 单 worker 执行两个 project，避免同时请求同一 standalone 服务造成不必要竞争。
...
还需以 `E2E_BASE_URL` 模式对 standalone production server 运行一次 `npm run test:e2e`，证明 CI 路径可用。桌面和移动两个 project 均通过、无未解释 Console error、工作区无测试产物污染，才视为完成。

## 风险与边界

- 真实搜索排序是 E2E 契约的一部分；如果质量门禁有意调整“如何面对困境”的第一条结果，E2E 期望需随经过评审的新契约更新。
- 两个真实浏览器 project 会增加 CI 时间和 Chromium 下载体积，但范围限制为单浏览器引擎和一条主流程。
